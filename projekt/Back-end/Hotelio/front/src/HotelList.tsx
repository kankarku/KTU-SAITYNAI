...
function HotelList(props: any) {
  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await authAxios.post<Hotel>(
        UrlManager.getCreateHotelEndpoint(
          hotelToCreate.name,
          hotelToCreate.location,
          hotelToCreate.rooms
        ),
        {
          name: hotelToCreate.name,
          location: hotelToCreate.location,
          rooms: hotelToCreate.rooms,
        }
      );
    ...
    const fetchHotels = async () => {
      try {
        const response = await authAxios.get<Hotel[]>(
          UrlManager.getAllHotelsEndpoint()
        );
  ...
  let body = isLoading ? (
    ...
    hotels.map((hotel, index) => {
      return (
        <Link key={index} to={`/hotel/${hotel.id}`} className="genre-item">
          {hotel.name}
        </Link>
      );
    })
