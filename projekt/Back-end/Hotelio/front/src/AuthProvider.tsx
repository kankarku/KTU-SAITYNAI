...
const AuthStateContext = createContext<AuthenticationState | null>(
  initialState
);
...
async function login(dispatch: AuthDispatch, request: LoginRequest) {
  const route = UrlManager.getLoginEndpoint();

  try {
    const getTokenResponse = await axios.post<AuthenticatedResponse>(
      route,
      request
    );
    if (getTokenResponse.status != 200 || !getTokenResponse.data.accessToken) {
      dispatch({
        user: null,
        status: ContextStatus.Rejected,
        error: "Missing access token, refresh token or wrong status code",
      });
      return;
    }
...
async function register(
  ...
  const route = UrlManager.getRegisterEndpoint();
...
async function refreshToken(
  dispatch: AuthDispatch,
  user: User
): Promise<AuthenticatedResponse | null> {
